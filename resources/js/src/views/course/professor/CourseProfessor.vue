                    ...
                    <b-card class="mb-2 border" no-body :key="item.id" v-if="(sortBy.value == 'assignments' && item.isAssignment) || (sortBy.value == 'quizzes' && item.isExam) || (sortBy.value == 'all')">
                      ...
                      <b-card-body>
                        <b-button-toolbar>
                          ...
                          <b-button variant="gradient-dark mt-1 ml-auto" size="sm" v-if="item.isAssignment && user.id == item.user_id" :to="{ name: 'assignment-edit', params: { id: state.assignmentsObj[item.id][0].assignment_id } }">
                            <feather-icon
                              icon="EditIcon"
                              size="16"
                              class="align-middle"
                          ...
                          <b-button variant="gradient-dark mt-1 ml-auto" size="sm" v-else-if="item.isExam && user.id == item.user_id" :to="{ name: 'quiz-edit', params: { id: state.quizzesObj[item.id][0].exam_id } }">
                            <feather-icon
                              icon="EditIcon"
                              size="16"
                              class="align-middle"
                        ...
                        <p>
                          <strong class="text-primary">
                            Sections<span v-for="section in item.sections" :key="section.id"> | {{section}} {{section.id}}</span>
                          </strong>
                        </p>
                        ...
                        <b-button class="mr-1" variant="gradient-primary" :href="state.assignmentsObj[item.id][0].file_name" v-else-if="item.isAssignment">
  ...
  setup(props, { emit }) {
    ...
    const {
      ...
      sortBy, sortByOptions,
		...
		fetchCourseProfessor({ id: courseId });
