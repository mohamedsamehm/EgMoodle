...
export default {
  ...
  created() {
    this.obj['student_id'] = this.user.id;
    try {
      axios.post('api/schedule/student', this.obj).then(res => {
        for (const [key, value] of Object.entries(res.data[0])) {
          this.schedulesObjStudent.push(value);
        }
        ...
        var today = new Date();
        this.schedulesObjStudent.forEach(element => {
