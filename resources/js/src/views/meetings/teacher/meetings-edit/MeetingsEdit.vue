          ...
          <b-row>
            <b-col md="6">
              <validation-provider #default="validationContext" rules="required" name="Title">
                ...
                <b-form-group>
                  <b-form-input
                    v-model="state.meeting.teams_url"
                    :state="getValidationState(validationContext)"
            ...
            <b-col md="6">
              ...
              <validation-provider #default="validationContext" rules="required" name="Sections">
                ...
                <b-form-group :state="getValidationState(validationContext)">
                  <v-select
                    v-model="state.meeting.course.selectedSections"
                    :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                    :options="state.meeting.course.sections"
                    label="section_name"
                    multiple
                  />
                  <b-form-invalid-feedback :state="getValidationState(validationContext)">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onSubmit = () => {
      state.meeting.course_id = state.meeting.course.course_id;
      state.meeting.sections = [];
      state.meeting.course.selectedSections.forEach(element => {
        state.meeting.sections.push(element.section_id);
      });
			loading.value = true;
      store.dispatch('meetings/updateMeeting', state.meeting).then((res) => {
				loading.value = false;
