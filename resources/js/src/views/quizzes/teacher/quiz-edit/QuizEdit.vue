          ...
          <b-row>
            <b-col md="6">
              ...
              <validation-provider #default="validationContext" rules="required" name="Sections">
                ...
                <b-form-group :state="getValidationState(validationContext)">
                  <v-select
                    v-model="state.quiz.course.selectedSections"
                    :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                    :options="state.quiz.course.sections"
                    label="section_name"
                    multiple
                  />
                  <b-form-invalid-feedback :state="getValidationState(validationContext)">
      ...
      <b-card no-body>
        ...
        <b-card-body class="p-2">
          ...
          <app-collapse accordion type="margin">
            <app-collapse-item v-for="(question, index) in questions" :key="index" :title="'Question ' + (index+1)">
              <b-row>
                <b-col md="6">
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onSubmit = () => {
      state.quiz.course.selectedSections.forEach(element => {
        state.quiz.sections.push(element.section_id);
      });
      state.quiz.course_id = state.quiz.course.course_id;
			loading.value = true;
      store.dispatch('quizzes/updateQuiz', state.quiz).then((res) => {
				loading.value = false;
    ...
    const {
      ...
      fetchQuiz,
		...
		fetchQuiz({ id: router.currentRoute.params.id });

    setTimeout(() => {
      questions.value = state.quiz.questions;
    }, 1000);
